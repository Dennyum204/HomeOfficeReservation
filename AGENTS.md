...
- HO-013 owner maintenance is operator-only: read docs/HO-013-OWNER-BOOTSTRAP.md and ADR-014. Preserve legacy admin-only bootstrap, credentials and calendar data. All administrative writers must lock the organization and reload current authority within the transaction; never permit general self-role changes or self-approval.
- HO-014 invitations: read docs/HO-014-INVITATIONS.md and ADR-015. Reuse Identity codes, protect durable delivery material, serialize activation/resend/cancel with administrative changes, preserve terminal cancellation and exact command replay. Email capture is private; SMTP acknowledgement is not proof of reading. HO-015 administration UI and HO-016 redesign remain separate; PR #37 remains draft/incomplete.
