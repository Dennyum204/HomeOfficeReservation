...
class TaskPage {
  ...
  @override
  String toString() => 'TaskPage[calendarVersion=$calendarVersion, items=$items, nextOffset=$nextOffset]';
  ...
  /// Clones this instance of [TaskPage] and returns a new one where some of the
  ...
  static TaskPage? fromJson(dynamic value) {
    if (value is Map) {
      ...
      assert(() {
        assert(json.containsKey(r'calendarVersion'), 'Required key "TaskPage[calendarVersion]" is missing from JSON.');
        assert(json[r'calendarVersion'] != null, 'Required key "TaskPage[calendarVersion]" has a null value in JSON.');
        ...
        assert(json.containsKey(r'nextOffset'), 'Required key "TaskPage[nextOffset]" is missing from JSON.');
        return true;
      }());

      return TaskPage(
        calendarVersion: mapValueOfType<int>(json, r'calendarVersion')!,
        items: TaskView.listFromJson(json[r'items']),
