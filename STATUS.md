...
PR #53 em revisão draft. Login, troca de conta, pedido, decisão, caixa interna e restauro de sessão após reinício do processo passaram no Samsung em 14/09/2026. Aviso flutuante FCM em preparação. Convite/ativação nativa e push físico ainda não concluídos.
...
Ativação SMTP na aplicação confirmada pelo recibo do operador em 2026-09-13: Brevo configurado, app saudável, fila novamente verificada durante pausa; zero mensagens enviadas pelo procedimento e nenhum backup executado. Rede smtp_egress apenas na app, saída geral não restrita ao destino; override da CA privada removido e configuração anterior preservada com comando de recuperação privado. Entrega via aplicação/MailKit continua não testada. PR #37 draft e HO-012 incompleta.

Validação manual SMTP pela aplicação confirmada pelo responsável: convite enviado pela Administração, email recebido e ativação concluída. Captura mostra membro de teste Colaborador, Ativo e Aceite/conta ativada, sem chefe associado. Evidência confirma o percurso de entrega/aceitação na aplicação alojada no Pi; não comprova cabeçalhos DKIM/DMARC nem todos os casos de falha de entrega. Endereço e captura permanecem privados. Nenhum backup executado para esta validação. PR #37 mantém-se draft; HO-012 permanece aberta.
