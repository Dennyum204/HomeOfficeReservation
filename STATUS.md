...
- HO-000: `in_progress`; documentação e tracking preparados. [Issue HO-000 #1](https://github.com/Dennyum204/HomeOfficeReservation/issues/1). PR, CI remota e proteções estão a ser concluídos.
- GitHub CLI 2.100.0 autenticado como Dennyum204, com credencial no keyring, protocolo HTTPS e acesso ADMIN ao repositório. `gh auth setup-git --hostname github.com` e `gh auth status` executados; bloqueio de autenticação resolvido. O executável foi mantido em `C:/Users/ferna/.local/bin/gh.exe`, fora do diretório temporário, para o helper Git continuar disponível.
