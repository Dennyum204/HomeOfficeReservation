...
- Evidência local HO-003: PostgreSQL 18.6 real preparado sem Docker, migração inicial aplicada e três contas sintéticas privadas provisionadas; 8 testes de integração PG + 2 testes API passaram; Web format/typecheck/lint/build e 3 testes UI simulados, 8 E2E reais; Flutter análise e 6 testes simulados, ensaio nativo Android com API/PG e secure storage reais (login/restauro/expiração/logout/troca de conta). Keyring cifrado partilhado entre dois hosts validado; APK release sem assinatura compilado. O estado verificável do SHA final está em [PR #29/checks](https://github.com/Dennyum204/HomeOfficeReservation/pull/29/checks); o relatório do PR regista os resultados remotos e só permite retirar draft com os cinco gates verdes.
