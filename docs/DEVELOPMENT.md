...
## Notificações em HO-007

[Setup, migração, duas contas e recuperação](HO-007-NOTIFICATIONS.md). `Notifications.PushProvider` começa Disabled, com worker e caixa ativos; normal development/CI não pede Firebase, Microsoft nem credenciais externas. Local é simulação explícita e só Development/Testing; Fcm tem setup privado separado e gate de entrega real. `python scripts/check_fcm_build.py` compila recursos Android sintéticos sem instalar/enviar. A CI Android preserva autenticação e acrescenta inbox real e esta compilação; quatro nomes obrigatórios mantidos, sem execução iOS. Não alterar/resetar as contas privadas existentes para repetir ensaios.
