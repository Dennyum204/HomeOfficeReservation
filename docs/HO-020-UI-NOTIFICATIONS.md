...
O percurso nativo `integration_test/notifications_test.dart` foi atualizado para abrir o contexto pela UI (o botão manual deixou de existir) e passou no emulador local com PostgreSQL real: leitura no servidor, contador, remoção da caixa Por ler, filtro Já lidas e pedido inalterado. Comando: `flutter drive -d <emulador-isolado> --driver test_driver/integration_test.dart --target integration_test/notifications_test.dart --dart-define-from-file=<ficheiro-privado>`.
