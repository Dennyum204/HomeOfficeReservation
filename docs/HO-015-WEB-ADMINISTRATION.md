...
Resultados locais em 2026-09-11: 69 testes de integração PostgreSQL e 4 testes API passaram; contratos coincidem (178 ficheiros), validador documental e configuração nativa passaram. Web: format/typecheck/lint/build, 13 testes unitários, 4 percursos de autenticação e 30 E2E desktop/estreito passaram. Flutter: análise, 42 testes e compilação debug da aplicação normal passaram. Os quatro checks remotos do commit entregue são verificados e ligados no PR, sem inferir resultados remotos destes ensaios locais.
...
No primeiro head com tracking final (`8e4b593`), [Android remoto](https://github.com/Dennyum204/HomeOfficeReservation/actions/runs/34580614162) falhou ao guardar o rascunho: o log mostrava `PlanningFailure.forbidden`, sem distinguir HTTP 401/403; os outros três checks passaram. Foi reproduzida, com teste inicialmente vermelho, uma janela de expiração entre preflight e escrita. A correção no repository permite uma única repetição **apenas após 401 explícito**, revalidando sessão/ator e conservando corpo/chave. Não repete 403, transporte, timeout ou 5xx. Os testes simulados verificam renovação, limite de uma repetição e recusa de troca de ator; os testes de resposta incerta continuam a exigir recuperação explícita.

A CI mantém access=5s/refresh=30s e acrescenta `TEST_EXPIRE_PLANNING_WRITE=true` ao percurso de planeamento: atrasa seis segundos a primeira escrita já construída e exige 401 real seguido de uma única escrita bem-sucedida. A injeção do atraso é controlada; autenticação, rejeição, renovação e persistência são reais. Não aumenta durações nem remove asserções. Esta prova permite verificar a correção sem atribuir retroativamente um HTTP bruto ao log anterior.
