...
O responsável pediu ver destino/custo/agenda antes de autorizar os envios; ainda não foi executado `init`, upload, restauro externo ou instalação de timers. Gate privado bloqueia o lançador até aprovação explícita. Confirmação do kit é independente dessa autorização.
...
Antes do primeiro envio: preparar e confirmar o **kit de recuperação independente**: password restic, endpoint/bucket/prefixo/ID, versão/checksum do binário e este runbook. Guardar fora do Pi e com recuperação independente do PC; acesso Cloudflare/MFA/códigos de recuperação sob controlo do responsável. Token S3 pode ser reemitido após perda do Pi; perder a única password restic impede recuperar. A própria cópia cifrada não pode ser o único local da sua chave. O exemplo mantém flags falsas. O responsável confirmou Bitwarden/acesso noutro dispositivo; configuração privada regista `recovery_kit_confirmed=true`. `external_authorized=false` bloqueia instalação/init/envios até à nova aprovação final solicitada para esta agenda/destino/custo.
...
Estes comandos aguardam aprovação final da configuração revista e execução real; esta documentação não é evidência de sucesso.
...
`pi-preparation` acrescenta PostgreSQL real: captura → restic local cifrado → download → base nova → rejeição de sobrescrita → PFX/key ring recuperados → cookie existente/plano aprovado. Resultados remotos por commit no PR; não transferir sucesso local para CI. Quatro checks core preservados. **Kit independente confirmado pelo responsável; envio/restauro R2, agenda real e impacto sobre o Pi em uso permanecem por verificar.**
