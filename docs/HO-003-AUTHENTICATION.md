...
A compilação do teste precede o arranque do simulador e omite o dump verbose de definições Xcode; erros continuam filtrados. O limite de build é 600 s e o de boot 300 s. A CI agrupa os builds/ensaios debug antes da compilação release ARM64, evitando alternar debug/release/debug e a competição com o simulador durante o build do teste. A primeira tentativa Intel atingiu o antigo limite de 420 s durante o build, sem chegar a instalar o teste; não conta como validação funcional.
...
O entrypoint de testes iOS, apenas com `TEST_IOS_CONSOLE=true` fornecido pelo wrapper, escreve um marcador de arranque e consulta [`Service.getInfo`](https://api.dart.dev/dart-developer/Service/getInfo.html), enviando o URI loopback pelo [`stdout`](https://api.dart.dev/dart-io/Stdout-class.html) anexado. Fontes oficiais consultadas em 2026-09-09. O URI é redigido antes do log público e usado só em memória. A app de teste arranca sem `--start-paused`; `integrationDriver` recolhe os resultados retidos pelo binding, sem mudar as asserções. Este diagnóstico não existe no entrypoint normal nem em release. Permite distinguir arranque Dart/serviço disponível de uma falha da descoberta de logs; não declara resultados que não tenham sido observados.

O [logger iOS do Flutter 3.47.2](https://github.com/flutter/flutter/blob/3.47.2/engine/src/flutter/shell/platform/darwin/common/framework/Source/Logger.swift), consultado em 2026-09-09, usa syslog; o stdout direto do teste evita depender dessa via. Em `57c03d0`, o driver confirmou os testes passados, mas o gate falhou no wrapper por prazo durante processamento de diagnóstico verbose. O driver passa a usar output compacto e limite de 300 s. O timer só marca expiração se o comando ainda estiver ativo; um processo já terminado pode concluir a leitura do seu output. Além dos quatro testes de privacidade, duas regressões simuladas cobrem esse caso e garantem que um processo ativo além do prazo continua a falhar, mesmo com saída 0 após o sinal. Não substituem o resultado nativo e os builds do SHA final.
