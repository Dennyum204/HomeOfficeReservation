...
Data: 2026-09-12. Estado: adotado no ensaio; publicação autorizada e verificada. A decisão inicial previa autorização final, posteriormente concedida após CI verde. Complementa ADR-019, preservando o ensaio local/NAS como histórico.
...
[Evidência da publicação real](../HO-012-HTTPS-PUBLICATION.md): conector Pi, DNS do hostname único, TLS/cookies/CSRF/bearer, recuperação automática e limites verificados. Sem alteração das regras Identity ou dados; não equivale a aceitação final de produção.
