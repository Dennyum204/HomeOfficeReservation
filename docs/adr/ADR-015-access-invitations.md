...
- [Flutter: ciclo de vida de FutureBuilder](https://api.flutter.dev/flutter/widgets/FutureBuilder-class.html). A CI HO-014 expôs uma consulta de metadados iniciada antes de existir o widget consumidor em Definições; o resultado é agora capturado imediatamente para tratar erro mesmo fora dessa secção ou após dispose. Falhas continuam visíveis como offline/retry, com duas regressões que falhavam antes da correção.
