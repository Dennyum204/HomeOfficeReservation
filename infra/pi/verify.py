#!/usr/bin/env python3
"""Exercise the exact Pi stack on a disposable Linux CI host. This is NOT Pi evidence."""
...
def main():
    ...
    if platform.machine() not in ('aarch64', 'arm64'):
        raise RuntimeError('Native ARM64 runner required; CPU emulation is not allowed.')
    if os.environ.get('GITHUB_ACTIONS') != 'true' or os.environ.get('RUNNER_ARCH') != 'ARM64':
        raise RuntimeError('Destructive harness is restricted to a disposable GitHub ARM64 runner, never the Pi.')
    ...
    image = 'homeoffice-pi:ho012-' + sha
    base_images = []
    for upstream in ['node:24.20.0-bookworm-slim', 'mcr.microsoft.com/dotnet/sdk:10.0.400-noble', 'mcr.microsoft.com/dotnet/aspnet:10.0.11-noble']:
        subprocess.run(['docker', 'pull', '--platform', 'linux/arm64', upstream], check=True)
        meta = json.loads(subprocess.check_output(['docker', 'image', 'inspect', upstream]))[0]
        assert meta['Architecture'] == 'arm64' and meta['Os'] == 'linux'
        base_images.append({'source': upstream, 'id': meta['Id'], 'digests': meta['RepoDigests'], 'architecture': meta['Architecture']})
    ...
    for name, upstream in [('postgres', 'postgres:18.6-bookworm'), ('caddy', 'caddy:2.11.4-alpine'), ('mailpit', 'axllent/mailpit:v1.31.1')]:
        ...
        meta = json.loads(subprocess.check_output(['docker', 'image', 'inspect', tag]))[0]
        assert meta['Architecture'] == 'arm64' and meta['Os'] == 'linux'
        base_images.append({'source': upstream, 'id': meta['Id'], 'digests': meta['RepoDigests'], 'architecture': meta['Architecture']})
        images.append(tag)
    with tempfile.TemporaryDirectory(prefix='ho012-pi-ci-') as temporary:
        ...
        def trial(*args, ok=True):
            ...
            return result
        def dc(*args):
            return subprocess.check_output(['docker', 'compose', '-p', 'homeoffice-pi-trial', '-f', str(folder / 'compose.yaml'), *args], cwd=folder, stderr=subprocess.DEVNULL)
        ...
        def pressure():
            ...
            return {key: int(values.get(key, 0)) for key in ['pswpin', 'pswpout', 'oom_kill']}
        ...
        def request(browser, path, body=None, method=None, expected=200, csrf=False):
            ...
            req = urllib.request.Request('https://10.78.17.2' + path,
            ...
            if body is not None and response.status < 400:
                write_samples.append(round((time.monotonic() - started) * 1000, 1))
        ...
        def activate(name, browser):
            ...
            for _ in range(60):
                messages = json.load(urllib.request.urlopen('http://10.78.17.3:8025/api/v1/messages'))
                match = next((m for m in messages['messages'] if any(t['Address'] == email for t in m['To'])), None)
                if match:
                    message = json.load(urllib.request.urlopen('http://10.78.17.3:8025/api/v1/message/' + match['ID']))
        ...
        try:
            ...
            invite = {'email': credentials['manager']['email'], 'displayName': 'Chefia — ENSAIO Pi',
            ...
            # Short concurrency smoke, not a Pi capacity/load test or an SLA.
            ...
            evidence = {'commit': sha, 'environment': 'native ubuntu-24.04-arm CI, NOT Raspberry Pi hardware',
                ...
                'read_latency_ms_runner_only': samples, 'write_latency_ms_runner_only': write_samples,
                'pressure_before_runner_only': baseline_pressure, 'pressure_after_runner_only': pressure(), 'pi_installation': 'not performed', 'pi_capacity': 'unmeasured',
                'email': 'captured only; external relay absent', 'push': 'Disabled', 'independent_backup': 'not configured'}
            (ROOT / 'ho012-pi-evidence.json').write_text(json.dumps(evidence, indent=2) + '\n')
            (ROOT / 'ho012-pi-runner-metrics.txt').write_text(metrics)
            print(json.dumps(evidence))
        finally:
            # This fixed project exists only on this disposable runner; never invoke this harness on the user's Pi.
            dc('down', '--volumes')
    STAGE = 'image archive export'
    output = ROOT / 'ho012-pi-images'
    ...
    assert all(x['Architecture'] == 'arm64' and x['Os'] == 'linux' for x in metadata)
    manifest = {'commit': sha, 'platform': 'linux/arm64', 'native_build': True, 'dependencies': base_images, 'archive_sha256': hashlib.file_digest(archive.open('rb'), 'sha256').hexdigest(),
...
if __name__ == '__main__':
    try:
        ...
        print('Pi preparation CI failed at ' + STAGE + ': ' + type(error).__name__)
